...
class _DriverScannerPageState extends State<DriverScannerPage>
  ...
  /// Parses the QR data string generated by the bus company QR system
  /// Expected format: "ID: TRP-XXXXXXX-XXX | Trip: CityA to CityB | Bus: BUS-XXX"
  ...
  void _onDetect(BarcodeCapture capture) {
    if (!isScanCompleted) {
      ...
      if (barcodes.isNotEmpty) {
        ...
        final codeId = parsed['ID'] ?? code;

        if (_scannedStudents.any((s) => s.codeId == codeId)) {
          _showDuplicateDialog(codeId);
          return;
        }
        ...
        _showSuccessDialog(code, parsed);
      }
    }
  }
  ...
  void _showSuccessDialog(String rawCode, Map<String, String> parsed) {
    final codeId = parsed['ID'] ?? rawCode;
    final tripName = parsed['Trip'] ?? 'Unknown Trip';
    final busId = parsed['Bus'] ?? 'Unknown Bus';
